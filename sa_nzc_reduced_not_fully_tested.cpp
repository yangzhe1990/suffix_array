...
void _suffix_array_worker(T *pattern, int length, int new_length,
			  ...
			  int *sorted_suffix,
			  int *bucket_end, int *bucket_p) {
	int orig_new_length = new_length;
	gettimeofday(&tv, NULL);
	...
	for (s = length - 1; s >= 0; --s) {
		...
		if (i >= 0) {
			...
			if (last_sstar_deleg == equal_deleg) {
				--new_alphabet_size;
				if (tmp_int3 == equal_deleg)
					tmp_int3 ^= -1;
				new_sorted_suffix[j] = tmp_int3;
				// temporarily use new_sorted_suffix
				// to store the sorted substrings
			}
			else {
				last_sstar_deleg = equal_deleg;
				if (j < new_length)
					new_sorted_suffix[j] = tmp_int3;
				tmp_int3 = equal_deleg;
			}
			new_pattern[--j] = i;
...
if (do_recursion) {
	...
	for (i = 0; i < new_length; ++i)
		sorted_suffix[new_pattern[i]] = new_sorted_suffix[i];
	// shorten the new pattern and compute the new_alphabet_size.
	// Note that the last symbol $ should not be dropped!
	gettimeofday(&tv, NULL);
	fprintf(stderr, " %d prepare rec. at %d.%06d\n",
		length, tv.tv_sec, tv.tv_usec);
	tmp_int1 = 0;
	/* 0: drop next if the next is a uniq substr;
	 * 1: don't drop the next substr */
	new_length = 0;
	for (i = 0; i < orig_new_length; ++i) {
		tmp_int3 = new_orig_pos[i];
		tmp_int2 = (sorted_suffix[tmp_int3] < 0);
		if (tmp_int2)
			sorted_suffix[tmp_int3] ^= -1;
		if (tmp_int1 || tmp_int2)
			new_orig_pos[new_length++] = new_orig_pos[i];
		else {
			--new_alphabet_size;
			sorted_suffix[tmp_int3] = -1;
		}
	...
	j = -1;
	for (i = 0; sorted_suffix[tmp_int2 = new_pattern[i]] < 0; ++i) {
		new_sorted_suffix[new_length] = j;
		new_orig_pos[new_length++] = tmp_int2;
	}

	/* Prepare for generating the new pattern string and compute
	 * the new_bucket_end, new_bucket_p */
	int *new_bucket_end =
	...
	j = 0;
	tmp_int1 = sorted_suffix[new_pattern[i]];
	sorted_suffix[new_pattern[i]] = j;
	s = 1;
	for (++i; i < orig_new_length; ++i) {
		tmp_int2 = new_pattern[i];
		tmp_int3 = sorted_suffix[tmp_int2];
		if (tmp_int3 >= 0) {
			if (tmp_int3 > tmp_int1)
				new_bucket_end[j++] = s;
			++s;
			sorted_suffix[tmp_int2] = j;
			tmp_int1 = tmp_int3;
		}
		else {
			new_sorted_suffix[new_length] = j;
	...
	gettimeofday(&tv, NULL);
	fprintf(stderr, "init %d at %d.%06d\n", new_length, tv.tv_sec, tv.tv_usec);
	...
	for (i = 0; i < new_length; ++i) {
		c1 = this_chr;
		this_chr = sorted_suffix[new_orig_pos[i]];
	...
	i = 0;
	tmp_int2 = 0;
	while (j < orig_new_length) {
		tmp_int1 = new_sorted_suffix[j];
		while (i < new_length
		       && new_pattern[new_sorted_suffix[i]] <= tmp_int1)
			sorted_suffix[tmp_int2++] =
				new_orig_pos[new_sorted_suffix[i++]];
		if (i < new_length) {
			tmp_int1 = new_pattern[new_sorted_suffix[i]];
			while (j < orig_new_length
			       && new_sorted_suffix[j] < tmp_int1)
				sorted_suffix[tmp_int2++] = new_orig_pos[j++];
/* alternative
			for (tmp_int3 = j;
			     tmp_int3 < orig_new_length
				     && new_sorted_suffix[tmp_int3] < tmp_int1;
			     ++tmp_int3);
			tmp_int3 -= j;
			memcpy(sorted_suffix + tmp_int2, new_orig_pos + j,
			       tmp_int3 * sizeof(int));
			tmp_int2 += tmp_int3;
			j += tmp_int3;
*/
...
*/
	...
	// then fill in the sorted_suffix
	c0 = alphabet_size;
	b = sorted_suffix + length;
	int *bb;
	while (tmp_int2 > 0) {
		i = sorted_suffix[--tmp_int2];
		c1 = pattern[i];
		//if (c1 > c0)
		//	assert(0);
		if (c1 < c0) {
...
*/
	...
	while (b > sorted_suffix)
		*(--b) = 0;
/* alternative
	memset(sorted_suffix, 0, (b - sorted_suffix) * sizeof(int));
*/
	gettimeofday(&tv, NULL);
...
// This is the no reduction version. Reduce the new_pattern is useless in the
// first level.
template <typename T>
void _suffix_array_worker_no_reduction(T *pattern, int length, int new_length,
				       int *new_orig_pos,
				       int alphabet_size,
				       int *sorted_suffix,
				       int *bucket_end, int *bucket_p) {
...
void suffix_array(char *pattern, int length) {
	...
	_suffix_array_worker_no_reduction<char>(
		pattern, length, new_length, new_orig_pos,
