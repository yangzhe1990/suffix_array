...
void _suffix_array_init(char *pattern, int length, int *bucket_end) {
	gettimeofday(&tv, NULL);
...
int _suffix_array_sort_substring_l(T *pattern, int length, int new_length,
				   ...
				   int *bucket_end, int *bucket_p,
				   int *sorted_substrings,
				   int **p_new_pattern, int **p_new_bucket_end) {
...
void _suffix_array_main(T *pattern, int length,
	...
	if (s_count < ((length + 1) >> 1)) {
		...
		if (new_alphabet_size > 0) {
			...
			// merge reduced pos and the new_pattern.
			int t = new_length - 1;
			s = new_r_length - 1;
			int tmp;
...
void suffix_array(char *pattern, int length) {
	...
	_suffix_array_init(pattern, length, bucket_end);
	_suffix_array_main<char>(pattern, length, alphabet_size,
				 sorted_suffix,
				 bucket_end, bucket_p);
	free(bucket_end);
	free(bucket_p);
