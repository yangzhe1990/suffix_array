...
template <typename T>
void _suffix_array_main_ka_red(T *pattern, int length,
			       int alphabet_size,
			       int *sorted_suffix,
			       int *bucket_end, int *bucket_p) {
	gettimeofday(&tv, NULL);
...
// this is the no reduction version of NZC. NZC is better than KA at
// the first level of computation. Reduce the new_pattern is useless
// in the first level.
template <typename T>
void _suffix_array_main_nzc_no_red(T *pattern, int length, int new_length,
				   int *new_orig_pos,
				   int alphabet_size,
				   int *sorted_suffix,
				   int *bucket_end, int *bucket_p) {
...
if (do_recursion) {
	...
	/* Generate the new pattern string */
	gettimeofday(&tv, NULL);
	...
	for (i = 0; i < new_length; ++i)
		new_pattern[i] = sorted_suffix[new_orig_pos[i] >> 1];

	/* recursion */
	_suffix_array_main_ka_red<int>(new_pattern, new_length, new_alphabet_size,
				       new_sorted_suffix,
				       new_bucket_end, new_bucket_p);
...
int _suffix_array_init_nzc(unsigned char *pattern, int length,
			   int alphabet_size,
			   int *sorted_suffix,
			   int *bucket_end, int *bucket_p,
			   int **new_orig_pos) {
	...
	*new_orig_pos = _new_orig_pos;
	...
	int same = 0;
	c = pattern;
	unsigned char c1 = c[0], this_chr, c0 = c1;
	int *b = sorted_suffix + bucket_p[c0];
	char r = 0;
	for (i = 1; i < length; ++i) {
		this_chr = *(++c);
...
void suffix_array(char *pattern, int length) {
	...
	int new_length;
	new_length = _suffix_array_init_nzc((unsigned char *)pattern, length, alphabet_size,
					    sorted_suffix,
					    bucket_end, bucket_p,
					    &new_orig_pos);
	_suffix_array_main_nzc_no_red<unsigned char>(
		(unsigned char *)pattern, length, new_length, new_orig_pos,
